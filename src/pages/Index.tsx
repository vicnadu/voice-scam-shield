...
import { saveGuestAnalysis, saveUserAnalysis } from "@/services/history";
import { LanguageSwitcher } from "@/components/LanguageSwitcher";
import { ThemeToggle } from "@/components/ThemeToggle";
import { HistoryTab } from "@/components/HistoryTab";
import { useAuth } from "@/hooks/useAuth";
import { useNavigate, Link } from "react-router-dom";
import { LogOut, User, Upload, History } from "lucide-react";
  ...
  const onAnalyze = async () => {
    ...
    try {
      ...
      if (data?.status === "ok") {
        ...
        try {
          if (isGuest) {
            saveGuestAnalysis(file.name, file.size, data);
          } else if (user) {
            await saveUserAnalysis(file.name, file.size, data);
          }
        } catch (historyError) {
